...
namespace DAO
{
    public class TuaSachDAO
    {
        ...
        {
            if (tuaSach == null || string.IsNullOrWhiteSpace(tuaSach.MaTuaSach))
                return false;

            // Get ID from MaTuaSach
            object? idObj = DataProvider.Instance.ExecuteScalar("SELECT ID FROM TuaSach WHERE MaTuaSach = @Ma LIMIT 1", new MySqlParameter("@Ma", tuaSach.MaTuaSach));
            if (idObj == null) return false;
            int idTuaSach = Convert.ToInt32(idObj);

            // Update main row by ID (use ID to be robust)
            string updateQuery = @"
                UPDATE TuaSach
                ...
                WHERE ID = @ID
            ";
            int count = DataProvider.Instance.ExecuteNonQuery(updateQuery,
                new MySqlParameter("@TenTuaSach", tuaSach.TenTuaSach),
                new MySqlParameter("@AnhBia", tuaSach.AnhBia ?? (object)DBNull.Value),
                new MySqlParameter("@ID", idTuaSach)
            );
            ...
            DataProvider.Instance.ExecuteNonQuery("DELETE FROM CT_TheLoai WHERE IDTuaSach = @ID", new MySqlParameter("@ID", idTuaSach));
            if (!string.IsNullOrWhiteSpace(tuaSach.TheLoai))
            {
                foreach (string theLoai in tuaSach.TheLoai.Split(new[] { ", " }, StringSplitOptions.RemoveEmptyEntries))
                {
            ...
            DataProvider.Instance.ExecuteNonQuery("DELETE FROM CT_TacGia WHERE IDTuaSach = @ID", new MySqlParameter("@ID", idTuaSach));
            if (!string.IsNullOrWhiteSpace(tuaSach.TacGia))
            {
                foreach (string tacGia in tuaSach.TacGia.Split(new[] { ", " }, StringSplitOptions.RemoveEmptyEntries))
                {
        ...
        public static BindingList<TuaSachDTO> Search(string keyword, int genreId = 0, int authorId = 0)
        {
            ...
            string query = @"
                SELECT
                    ...
                    GROUP_CONCAT(DISTINCT TG.TenTacGia SEPARATOR ', ') AS TacGia
                FROM TuaSach AS TS
                LEFT JOIN CT_TheLoai AS CT_TL ON TS.ID = CT_TL.IDTuaSach
                LEFT JOIN TheLoai AS TL ON CT_TL.IDTheLoai = TL.ID
                LEFT JOIN CT_TacGia AS CT_TG ON TS.ID = CT_TG.IDTuaSach
                LEFT JOIN TacGia AS TG ON CT_TG.IDTacGia = TG.ID
                WHERE TS.DaAn = 0
                    AND (@Keyword = '' OR LOWER(TS.TenTuaSach) LIKE CONCAT('%', LOWER(@Keyword), '%'))
                    AND (@GenreId = 0 OR EXISTS (
                        SELECT 1 FROM CT_TheLoai ct JOIN TheLoai t ON ct.IDTheLoai = t.ID
                        WHERE ct.IDTuaSach = TS.ID AND t.ID = @GenreId
                    ))
                    AND (@AuthorId = 0 OR EXISTS (
                        SELECT 1 FROM CT_TacGia ctg JOIN TacGia tg ON ctg.IDTacGia = tg.ID
                        WHERE ctg.IDTuaSach = TS.ID AND tg.ID = @AuthorId
                    ))
                GROUP BY TS.ID
            ";

            var dt = DataProvider.Instance.ExecuteQuery(query,
                new MySqlParameter("@Keyword", keyword ?? string.Empty),
